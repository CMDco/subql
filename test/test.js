const chai = require('chai'); 
const sinon = require('sinon');
const sinonChai = require('sinon-chai');

const {
  parse,
  validate,
} = require('graphql'); 

const {
  registerResolver,
  getRoot,
  registerType,
  parseSchema,
  handleSubscribe,
  handleDisconnect,
  triggerType
} = require('../src/subql.js');

chai.use(sinonChai);
const expect = chai.expect;
const assert = chai.assert;

describe('SubQL Tests', () => {
  describe('Unit Test :: registerResolver', () => {
    it('should throw an error when no arguments are passed in.', done => {
      done();
    });
    it('should throw an error when anonymous functions are passed in', done => {
      done();
    });
    it('should create keys for all functions passed in', done => {
      done();
    });
  });

  describe('Unit Test :: getRoot', () => {
    it('should return a root object', done => {
      done();
    });
  });

  describe('Unit Test :: wrapResolver', () => {
    it('should wrap functions according to type', done => {
      done();
    });
    it('should return a function that triggers the proper type', done =>{
      done();
    });
  });

  describe('Unit Test :: parseSchema', () => {
    it('should throw an error when nothing is passed in', done => {
      done();
    });
    it('should populate the operations object when a valid schema is passed in', done =>{
      done();
    });
  });
  describe('Unit Test :: handleDisconnect', () => {
    it('should remove something from connected object', done => {
      done();
    });
    it('should be refactored to another file', done => {
      done();
    });
  });
  describe('Unit Test :: triggerType', () => {
    it('should trigger socket communications based on type', done => {
      done();
    });
  })
  describe('Unit Test :: generateUniqueIdentifier', () => {
    it('should generate a Unique Identifier', done => {
      done();
    });
  });
});
